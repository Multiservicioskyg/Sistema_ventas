...
public function comprasfecha($fecha_inicio,$fecha_fin){
	...
	as proveedor, i.tipo_comprobante, i.serie_comprobante, i.num_comprobante, 
	i.total_compra,i.impuesto,i.estado FROM ingreso i INNER JOIN persona p 
	ON i.idproveedor=p.idpersona INNER JOIN usuario u ON i.idusuario=u.idusuario
...
public function ventasfechacliente($fecha_inicio,$fecha_fin,$idcliente){
	...
	cliente, v.tipo_comprobante,v.serie_comprobante, v.num_comprobante ,
	 ...
	 <='$fecha_fin' AND v.idcliente='$idcliente'";
	return ejecutarConsulta($sql);
}
...
public function ventasultimos_12meses(){
	$sql=" SELECT DATE_FORMAT(fecha_hora,'%M') AS fecha, SUM(total_venta)
	 AS total FROM venta GROUP BY MONTH(fecha_hora) ORDER BY fecha_hora DESC LIMIT 0,12";
	return ejecutarConsulta($sql);
}
